...
class Client {
 public:
  ...
  virtual bool DoWarm();
  virtual bool SetStart(uint64_t start);
...
inline bool Client::SetStart(uint64_t start) {
    workload_.SetStart(start);
    return true;
}
inline bool Client::DoWarm() {
  const std::string &table = workload_.NextTable();
  const std::string &key = workload_.NextSequenceKey();
  std::vector<DB::KVPair> result;
  return (db_.Read(table, key, NULL, result) == DB::kOK);
}
