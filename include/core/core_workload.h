...
class CoreWorkload {
 public:
  ...
  inline void NextTransactionKeyRange(uint64_t len, std::string &from, std::string &to);
  ...
  virtual void SetStart(uint64_t start) {
    delete key_generator_;
    key_generator_ = new CounterGenerator(start);
  }
...
inline void CoreWorkload::NextTransactionKeyRange(uint64_t len, std::string &from, std::string &to) {
  uint64_t key_num;
  do {
    key_num = key_chooser_->Next();
  } while (key_num > insert_key_sequence_.Last());
  from = BuildKeyName(key_num);
  to = BuildKeyName(key_num + len);
}
