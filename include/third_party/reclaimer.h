...
inline Reclaimer::HPIndex Reclaimer::MarkHazard(void* ptr) {
...
inline void Reclaimer::ReclaimNoHazardPointer() {
...
inline bool Reclaimer::Hazard(void* const ptr) {
  std::atomic<InternalHazardPointer*>& head = global_hp_list_.head;
  InternalHazardPointer* p = head.load(std::memory_order_acquire);
  do {
...
inline void Reclaimer::TryAcquireHazardPointer() {
  std::atomic<InternalHazardPointer*>& head = global_hp_list_.head;
  InternalHazardPointer* p = head.load(std::memory_order_acquire);
